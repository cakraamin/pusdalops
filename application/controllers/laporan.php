...
class Laporan extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->library(array('arey','page','excel'));
	...
	function index($kunci="kosong",$jenis=1,$bencana=0,$lokasi=0,$val=0,$waktu=1,$tahun=0,$bulan=0,$tanggal="00-00-0000",$short_by='id_bencana',$short_order='desc',$page=0)
	{
		...
		$url = 'laporan/index/'.$kunci.'/'.$jenis.'/'.$bencana.'/'.$lokasi.'/'.$val.'/'.$waktu.'/'.$tahun.'/'.$bulan.'/'.$tanggal.'/'.$short_by.'/'.$short_order.'/';	
		...
		{
			redirect('laporan/index/'.$kunci.'/'.$jenis.'/'.$bencana.'/'.$lokasi.'/'.$val.'/'.$waktu.'/'.$tahun.'/'.$bulan.'/'.$tanggal.'/'.$short_by.'/'.$short_order.'/'.($page - $per_page));		
		}
	...
	function submit()
	{
		...
		redirect('laporan/index/'.$kunci.'/'.$laporan.'/'.$bencana.'/'.$lokasi.'/'.$val.'/'.$waktu.'/'.$tahun.'/'.$bulan.'/'.$tanggal);
	}
	...
	function edit($id,$kuncine,$jenise,$bencanane,$lokasine,$val,$waktune,$tahune,$bulane,$tanggale,$sort_by,$sort_order,$page)
	{
		if($id == "")
		{
			$this->message->set('notice','Maaf parameter salah');

			redirect('laporan/index/'.$kuncine.'/'.$jenise.'/'.$bencanane.'/'.$lokasine.'/'.$val.'/'.$waktune.'/'.$tahune.'/'.$bulane.'/'.$tanggale.'/'.$sort_by.'/'.$sort_order.'/'.$page);
		}

		$data = array(	  	
			'main'			=> 'lapBencana',
			'ket'			=> 'Form Edit Data Laporan',
			'jenis'			=> 'Edit',
			'laporan'		=> 'select',
			'link'			=> 'update_bencana/'.$id,
			'kueri'			=> $this->mlaporan->editBencana($id),			
			'jenis_ben'		=> $this->mlaporan->getSelekBencana(),
			'lokasi'		=> $this->mlaporan->getSelekLokasine(),
			'idne'			=> $id,
			'kuncine'		=> $kuncine,
			'jenise'		=> $jenise,
			'bencanane'		=> $bencanane,
	...
	function hapus($id,$kuncine,$jenise,$bencanane,$lokasine,$val,$waktune,$tahune,$bulane,$tanggale,$sort_by,$sort_order,$page)
	{
		if($this->mlaporan->deleteBencana($id))
		{
